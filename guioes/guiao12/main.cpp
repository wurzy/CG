...
void renderText() {
	...
	glPushMatrix();
	glLoadIdentity();
	// para que as coordenadas sejam as dos pixeis da janela
	gluOrtho2D(0, w, h, 0);
	glMatrixMode(GL_MODELVIEW);

	glDisable(GL_DEPTH_TEST);

	glDisable(GL_LIGHTING); // desligar as luzes para o desenho do texto
	glColor3f(1.0f, 1.0f, 1.0f);

	glPushMatrix();
	glLoadIdentity();
	glRasterPos2d(10, 20); // posicao do texto em pixeis
...
unsigned char picking(int x, int y) {
	...
	// desenhar snowmen como no renderscene, dando um ID por cor a cada objeto
