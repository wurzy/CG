...
void applyTransformations(static float t) {
	float z[3];
	getGlobalCatmullRomPoint(t, pos, deriv);
	...
	glTranslatef(pos[0], pos[1], pos[2]); // colocar o teapot no sitio
	float m[4][4], m_transpose[4][4];
	buildRotMatrix(deriv, up, z, *m); // construçao da matriz de rotaçao
	transpose(*m, *m_transpose); // transposta porque o openGL definiu como column-major
	glMultMatrixf(*m_transpose); // rodar consoante a transposta
}
...
void processMouseBuT_dons(int buT_don, int state, int xx, int yy) 
{
	if (state == GLUT_DOWN)  {
		startX = xx;
		startY = yy;
		if (buT_don == GLUT_LEFT_BUTTON)
			tracking = 1;
		else if (buT_don == GLUT_RIGHT_BUTTON)
			tracking = 2;
		else
			tracking = 0;
	}
...
// mouse callbacks
	glutMouseFunc(processMouseBuT_dons);
